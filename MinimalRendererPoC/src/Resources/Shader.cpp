...
mrpoc::Shader::Shader(const std::string& computeFilename)
	: _vertexFilename("")
	, _fragmentFilename("")
	, _geometryFilename("")
...
void Shader::LoadParameters()
{
	...
	for (int uniform = 0; uniform < numActiveUniforms; uniform++)
	{
		...
		else
		{
			...
			_parameters[id] = unique_ptr<ShaderParameter>(param);
		}
	}
...
ShaderParameter* Shader::GetParameter(unsigned long long nameId)
{
	...
	_parameters[nameId] = unique_ptr<ShaderParameter>(param);
	return param;
}
