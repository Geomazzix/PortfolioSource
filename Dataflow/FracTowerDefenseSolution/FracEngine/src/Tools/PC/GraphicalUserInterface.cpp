...
namespace Frac {
    void GraphicalUserInterfacePreFrame::UpdateDocking()
    {
        bool p_open{ true };
    ...
    GraphicalUserInterfacePreFrame::GraphicalUserInterfacePreFrame() : ISystem(eStage::PreFrame) { }
    GraphicalUserInterfacePostFrame::GraphicalUserInterfacePostFrame(GraphicalUserInterface& a_GUI) : ISystem(eStage::PostFrame), m_GUI(a_GUI) { }

    void GraphicalUserInterfacePreFrame::Update(float a_deltaTime)
    {
    ...
    void GraphicalUserInterfacePostFrame::Update(float a_deltaTime)
    {
        m_GUI.UpdateGUI();
    }

	GraphicalUserInterface::GraphicalUserInterface() :
        m_menuItemViewFlag(0),
        m_GUIPreFrameSystem(),
        m_GUIPostFrameSystem(*this)
	{
        EngineCore* engineCore = &Frac::EngineCore::GetInstance();

		PCDevice* pcDevice = dynamic_cast<PCDevice*>(&engineCore->GetDevice());
        int videoResX = pcDevice->GetVideoResolutionWidth();
        int videoResY = pcDevice->GetVideoResolutionHeight();
        ...
        io.Fonts->AddFontFromFileTTF(FileIO::GetPathFromWildcard("[Fonts]NotoMono-Regular.ttf").c_str(), roundf(15.0f * m_guiScale));
        ...
        m_entityList = std::make_unique<EntityList>(engineCore->GetFracRegistry(), engineCore->GetSceneManager(), m_guiScale);
        m_componentInspector = std::make_unique<ComponentInspector>(engineCore->GetFracRegistry(), *dynamic_cast<EntityList*>(m_entityList.get()));

        //Turns the EntityList and Component inspector on by default
    ...
    void GraphicalUserInterface::UpdateGUI()
    {
        ...
        if (ImGui::BeginMenu("File"))
        {
            ...
            if (ImGui::MenuItem("Quit", nullptr, nullptr)) {
                Frac::EngineCore::GetInstance().Shutdown();
