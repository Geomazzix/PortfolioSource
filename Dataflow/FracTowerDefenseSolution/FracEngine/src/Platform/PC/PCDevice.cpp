...
namespace Frac
{
	PCDevice::PCDevice(Renderer& renderSystem, TextRenderSystem& textRenderSystem) :
	...
	void PCDevice::Initialize(unsigned width, unsigned height)
	{
		...
		m_window = glfwCreateWindow(width, height, "FracEngine Window", primaryMonitor, nullptr);
	...
	unsigned PCDevice::GetVideoResolutionWidth() const
	{
		return m_videoMode->width;
	}

	unsigned PCDevice::GetVideoResolutionHeight() const
	{
	...
	void PCDevice::Framebuffer_size_callback(GLFWwindow* window, int width, int height)
	{
		...
		device->m_textRenderSystem.ResizeViewport(static_cast<unsigned>(width), static_cast<unsigned>(height));
