...
std::unordered_map<JoystickButtons, int> virtualMapping =
{
	...
	{ JOYSTICK_BUTTON_Y,			GLFW_KEY_B },
	{ JOYSTICK_BUTTON_LB,			GLFW_KEY_Z },
	{ JOYSTICK_BUTTON_RB,			GLFW_KEY_X },
	...
	for (auto& i : m_joyState)
	{
		...
		if (joy >= 256)
		{
			...
			for (int j = 0; j < JOYSTICK_BUTTON_COUNT; j++)
			{
				...
				if (state.Buttons[j])
				{
					m_lastUsedJoystick = GetType(joy);
				}
			}
		}
		else
		{
			...
			if (glfwGetGamepadState(joy, &gamepadState))
			{
				...
				for (int j = 0; j < JOYSTICK_BUTTON_COUNT; j++)
				{
					const bool result = state.Buttons[j];
					if (result)
					{
						m_lastUsedJoystick = GetType(joy);
						break;
...
void GLFWInput::UpdateMouse(float dt)
{
	double mx, my;
...
void GLFWInput::AddJoystick(int joy)
{
	...
	else if (name == "Switch Pro Controller")	state.Type = JoystickType::JOYSTICK_TYPE_NINTENDO_DUAL_JOYCONS; //Not sure.. might be fullkey?
...
{
	const bool result = (glfwGetKey(m_windowPtr, key) ?
		(m_keyOnce[key] ? false : (m_keyOnce[key] = true)) :
		(m_keyOnce[key] = false));

	if (result)
		m_lastUsedJoystick = JoystickType::JOYSTICK_TYPE_INVALID;
...
{
	const bool result = glfwGetKey(m_windowPtr, key) == GLFW_PRESS;
