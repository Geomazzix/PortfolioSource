...
namespace CRT
{
	Box::Box() : Primitive()
	{
		m_Bounds[0] = glm::vec3(-0.5f);
		m_Bounds[1] = glm::vec3(0.5f);
	}
	...
	bool Box::Intersect(const Ray& ray, HitInfo& hit, const float maxRayLength)
	{
		...
		if (t0x > t1x) std::swap<float>(t0x, t1x);
		...
		if (t0y > t1y) std::swap<float>(t0y, t1y);
		...
		if (t0z > t1z) std::swap<float>(t0z, t1z);
	...
	glm::vec3 Box::GetNormal(const glm::vec3& point)
	{
		...
		if (abs(localPoint.x - m_Bounds[0].x) < 0.01f) return glm::vec3(-1.0f, 0.0f, 0.0f);
		if (abs(localPoint.x - m_Bounds[1].x) < 0.01f) return glm::vec3(1.0f, 0.0f, 0.0f);

		if (abs(localPoint.y - m_Bounds[0].y) < 0.01f) return glm::vec3(0.0f, -1.0f, 0.0f);
		if (abs(localPoint.y - m_Bounds[1].y) < 0.01f) return glm::vec3(0.0f, 1.0f, 0.0f);

		if (abs(localPoint.z - m_Bounds[0].z) < 0.01f) return glm::vec3(0.0f, 0.0f, -1.0f);
		if (abs(localPoint.z - m_Bounds[1].z) < 0.01f) return glm::vec3(0.0f, 0.0f, 1.0f);
